...
pot(knife).table(used, notmurderer, notmurderer)=0.32;  
pot(knife).table(used, notmurderer, murderer)   =0.04;
pot(knife).table(used, murderer,    notmurderer)=0.64;
pot(knife).table(used, murderer,    murderer)   =0.00;
...
disp('p(butler,maid|knife=used):')
disptable(condpot(setpot(jointpot,knife,used),[maid,butler]),variable);
